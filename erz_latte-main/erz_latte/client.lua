...
print("[ERZ-DEVELOPMENT] Lavoro Mucche Caricato!")
...
Citizen.CreateThread(function()
    ...
    TriggerEvent('gridsystem:registerMarker', {
        ...
        msg = 'Premi [E] per aprire il menu',
        control = 'E',
        type = 20,
    ...
    TriggerEvent('gridsystem:registerMarker', {
        ...
        msg = 'Premi [E] per depositare il veicolo',
        control = 'E',
        type = 23,
        color = {r = 255, g = 255, b = 255},
        action = function()
    ...
    elements = elements 
        ...
        if data.current.value ~= nil then
            ...
            ESX.Game.SpawnVehicle(Config.Impiego.vehicle,vector3(Config.Impiego.spawnpoint.x,Config.Impiego.spawnpoint.y,Config.Impiego.spawnpoint.z),Config.Impiego.spawnpoint.h, function(veh)
                ...
                TriggerEvent('erz_custom:vehcall', txt)
                                    ...
                                    if IsControlJustReleased(0, 51) then
                                        exports['mythic_progbar']:Progress({
                                            ...
                                            if not cancelled then
                                                TriggerServerEvent('erz_lavoronowl:raccogli', v.RaccoltaLavori.item, v.RaccoltaLavori.count, v.RaccoltaLavori.notify)
                                            end
                                        end)
                                    end
                                end
                                ...
                                if posizionePro < 3.0 then
                                    if IsControlJustReleased(0, 51) then
                                        exports['mythic_progbar']:Progress({
                                            ...
                                            if not cancelled then
                                                TriggerServerEvent('erz_lavoronowl:processa', v.ProcessoLavori.item, v.ProcessoLavori.count, v.RaccoltaLavori.item, v.ProcessoLavori.notify)
                                            end
                                        end)
                                    end
                                end
...
Citizen.CreateThread(function()
    for i = 1, #Config.BlipsInMap do
        blip = AddBlipForCoord(Config.BlipsInMap[i].Coords)
        SetBlipSprite(blip, Config.BlipsInMap[i].Sprite)
        SetBlipDisplay(blip, 4)
        SetBlipScale(blip, Config.BlipsInMap[i].Scale)
        SetBlipColour(blip, Config.BlipsInMap[i].Color)
        SetBlipAsShortRange(blip, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(Config.BlipsInMap[i].Nome)
        EndTextCommandSetBlipName(blip)
    end
end)